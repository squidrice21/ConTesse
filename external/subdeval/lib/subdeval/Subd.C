...
#include "subdeval/Subd.h"
#include "subdeval/SubdEigenData.h"
#include "subdeval/IntPairMap.h"
...
namespace {
    ...
    Vec3 operator/ (double c) const	{ return *this * (1/c); }
    Vec3& operator+= (const Vec3& c) { return *this = *this + c; }
    Vec3& operator-= (const Vec3& c) { return *this = *this - c; }
    Vec3& operator*= (double c) { return *this = *this * c; }
    Vec3& operator/= (double c) { return *this = *this * (1/c); }
    friend Vec3 operator* (double c, const Vec3& v) { return v * c; }
...
class SubdInternal {
    public:
    SubdInternal(int nverts, const double* verts,
...
Subd::Subd(const Subd& input):
  impl(new SubdInternal(*input.impl))
{ }

Subd::Subd(int nverts, const double* verts,
    int nfaces, const int* nvertsPerFace, const int* faceverts)
...
bool Subd::eval(int faceid, double u, double v, double * p, double * dPdU, double * dPdV)
{
    ...
    Vec3 dPdVvec(0,0,0);
    if(dPdU)  {
        dPdUvec = Vec3(dPdU[0], dPdU[1], dPdU[2]);
    }
    if(dPdV) {
        dPdVvec = Vec3(dPdV[0], dPdV[1], dPdV[2]);
    }
...
const double* Subd::verts()             { return (double*)impl->verts(); }
const int* Subd::nvertsPerFace()       { return impl->nvertsPerFace(); }
const int* Subd::faceverts()           { return impl->faceverts(); }
const double* Subd::normals()           { return (double*)impl->normals(); }
const double* Subd::limitverts()        { return (double*)impl->limitverts(); }


SubdInternal::SubdInternal(int nverts, const double* verts,
    int nfaces, const int* nvertsPerFace, const int* faceverts)
{
    ...
    Vec3& p, Vec3& dPdU, Vec3& dPdV)
{
    ...
    if (u <= ROUNDING_THRESHOLD) 
        u = ROUNDING_THRESHOLD;
    if (v <= ROUNDING_THRESHOLD) 
        v = ROUNDING_THRESHOLD;

    int n = (int)floor(std::min( -log2(u),-log2(v) ))+1;
    //double pow2 = pow(2.0,n);
    //unsigned long long pow2 = 1 << n;
    //u *= pow2; v *= pow2;
    u = pow(2.0, n + log2(u));
    v = pow(2.0, n + log2(v));
    ...
    // eigenvals++;
    // coeffs += 16; 
    // for (const Vec3* cp = &Cp[1], * cpend = cp + K; cp != cpend; cp++) {
    ...
    // change to log space
    dPdU.x = ((dPdU.x > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdU.x)));
    dPdU.y = ((dPdU.y > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdU.y)));
    dPdU.z = ((dPdU.z > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdU.z)));

    dPdV.x = ((dPdV.x > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdV.x)));
    dPdV.y = ((dPdV.y > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdV.y)));
    dPdV.z = ((dPdV.z > 0) ? 1 : -1) * pow(2.0, n + log2(abs(dPdV.z)));
    ...
    Vec3& p, Vec3& dPdU, Vec3& dPdV)
{
    ...
    if (u <= ROUNDING_THRESHOLD) u = ROUNDING_THRESHOLD;
    if (v <= ROUNDING_THRESHOLD) v = ROUNDING_THRESHOLD;
    ...
    p = dPdU = dPdV = Vec3(0.0, 0.0, 0.0);
    std::vector<double> eigenX(K), dXdU(K), dXdV(K);
    double *x = eigenX.data(), *dxdu = dXdU.data(), *dxdv = dXdV.data();
...
bool SubdInternal::eval(int faceid, double u, double v, Vec3& p, Vec3& dPdU, Vec3& dPdV)
{
    ...
    int fvertid = _faceVertIdOffsets[faceid];
    std::vector<int> valences(numverts);
    std::vector<bool> boundary(numverts);
    ...
    if (valences[cell] == 4) {
        ...
        // first vertex (EV)	
        std::vector<int> order(n);   		// this maps my traversal order to Stam order
        order[0] = 4; order[1] = 2;
        for (int i = 0; i < n-2; i++) 
        order[i+2] = 2*(n-i);
