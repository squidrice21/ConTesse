...
bool saveObjCorners(const char* filename, Subd* subd)
{
...

    {
        ...
        for (int i = 0; i < subd->nfaces(); i++, nv++) {
            for (int j = 0; j < *nv; j++) {
                ...
                subd->eval(i, uv[0], uv[1], pos, nullptr, nullptr);
...
bool saveObj(const char* filename, Subd* subd, bool limit)
{
    ...
    const double* v = limit ? subd->limitverts() : subd->verts();
    for (int i = 0; i < subd->nverts(); i++, v += 3) {
    fprintf(file, "v %g %g %g\n", v[0], v[1], v[2]);
    }
    
    const double* n = subd->normals();
...
int main(int argc, char** argv)
{
    ...
    //subd.subdivide();
    saveObj("shayan-limit.obj", &subd, true);
    saveObj("shayan.obj", &subd, false);
    saveObjCorners("shayan-corners.obj", &subd);
