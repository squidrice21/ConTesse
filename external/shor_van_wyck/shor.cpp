...
bool to_viz_failures = false;
// bool to_viz_failures = true;

using namespace std;

double orientation(const Eigen::Matrix<double, 3, 2> &P) {
  Eigen::Vector2d a{P(0, 0), P(0, 1)};
  Eigen::Vector2d b{P(1, 0), P(1, 1)};
  Eigen::Vector2d c{P(2, 0), P(2, 1)};
  return static_cast<double>(igl::predicates::orient2d(a, b, c));
}

void set_rotation_index(const Eigen::MatrixXd &uv, const Eigen::MatrixXi &F,
                        Eigen::VectorXi &R, int offset) {
  Eigen::VectorXi bd;
...
void add_triangle(Eigen::MatrixXi &F, int i, int j,
...
bool weakly_self_overlapping(const Eigen::MatrixXd &P, const Eigen::VectorXi &R,
                             Eigen::MatrixXi &F) {
  std::cout << "weakly self overlapping test" << std::endl;
  const int N = P.rows();
  auto add_to_table = [](const Eigen::MatrixXd &P, const Eigen::VectorXi &R,
                           std::vector<std::vector<Angle>> &FA,
                           std::vector<std::vector<Angle>> &LA, int i, int k,
                           int j) {
    ...
    // if ((Fr.r <= R(i)) && (La.r <= R(j)) && (k1 + k2 + k3).r <= R(k)) {
      // Fix: the rotation indices have to match exactly if it's the last
      // round of DP
      // {
      //   if (i == (j + 1) % N) {
      //     if (!((Fr.r == R(i)) && (La.r == R(j))))
      //       return false;
      //   }
      // }
      ...
      if ((Fr.r <= R(i)) && (La.r <= R(j)) && (k1 + k2 + k3).r != R(k)) {
        // std::cout << "Unsatisfied R: " << k << std::endl;
        float viz_i = i;
        int viz_j = (j < i) ? j + P.rows() : j;
        float viz_N = P.rows();

        // if ((viz_j - viz_i) >= viz_N / 3) {
        //   // Viz
        //   igl::opengl::glfw::Viewer vr;
        //   Eigen::VectorXi II = R;
        //   II.setZero();
        //   II[k] = 1;
        //   plot_polygon(vr, II, P);
        //   vr.launch();
        //   // Viz ends
        // }
      }
  ...
  for (int i = 0; i < N; i++) {
    Q[i][(i + 1) % N] = 1;
    FA[i][(i + 1) % N] =
        Angle(P.row((i + 1) % N), P.row(i), P.row((i + 1) % N));
    LA[i][(i + 1) % N] = Angle(P.row(i), P.row((i + 1) % N), P.row(i));
  }
  int h = -1;
  std::vector<std::vector<double>> A(N, std::vector<double>(N, -1)); // Unused.
  int max_d = 2;
...
void drop_colinear(const Eigen::MatrixXd &P, const Eigen::VectorXi &R,
                   Eigen::VectorXi &B, Eigen::MatrixXd &mP,
                   Eigen::VectorXi &mR) {
  int dropped = 0;
  int N = P.rows();
  std::vector<int> Bv;
  for (int i = 0; i < N; i++) {
    ...
    if (R(v) != 0 ||
        orientation(T) != 0) { // non-colinear or rotate index nonzero
      Bv.push_back(v);
    } else
      dropped++;
  }
...
void add_colinear(const Eigen::MatrixXd &P, const Eigen::MatrixXi &nF,
                  const Eigen::VectorXi &B, Eigen::MatrixXi &F) {
  int added = 0;
  ...
  for (int i = 0; i < nF.rows(); i++) {
    ...
    for (int k = 0; k < 3; k++) {
      ...
      if ((b - a == 1 || a - b == sN - 1) &&
          (B(b) - B(a) != 1 &&
           B(a) - B(b) != P.rows() - 1)) { // edge need split
...
void subdivide_polygon(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
...
void simplify_triangulation(const Eigen::MatrixXd &V_i,
...
bool Shor_van_wyck(const Eigen::MatrixXd &P, const Eigen::VectorXi &R,
                   const std::string flags, Eigen::MatrixXd &V,
                   Eigen::MatrixXi &F, bool do_refine) {
