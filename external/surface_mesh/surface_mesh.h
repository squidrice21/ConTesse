...
inline bool write_obj(const Surface_mesh& mesh, std::ostream& stream);
...
add_face(const std::vector<Vertex>& vertices)
{
    ...
    for (i=0, ii=1; i<n; ++i, ++ii, ii%=n)
    {
        ...
        {
            std::cerr << "Surface_meshT::add_face: complex vertex " << std::to_string(vertices[i].idx()) <<"\n";
            return Face();
        }
        ...
        if (!is_new[i] && !is_boundary(halfedges[i]))
        {
            std::cerr << "Surface_meshT::add_face: complex edge " << 
                std::to_string(vertices[i].idx()) << ", " << std::to_string(vertices[ii].idx()) <<"\n";
            return Face();
        }
    }
...
bool read_obj(Surface_mesh& mesh, const std::string& filename)
{
    ...
    while(in && !feof(in) && fgets(s, 200, in))
    {
        ...
        else if (strncmp(s, "f ", 2) == 0)
        {
            ...
            if (with_tex_coord && !all_tex_coords.empty()) {
              Surface_mesh::Halfedge_around_face_circulator h_fit =
                  mesh.halfedges(f);
              Surface_mesh::Halfedge_around_face_circulator h_end = h_fit;
              unsigned v_idx = 0;
              do {
                if (v_idx >= halfedge_tex_idx.size()) continue;
                tex_coords[*h_fit] =
...
bool write_obj(const Surface_mesh& mesh, const std::string& filename) {
    ...
    return write_obj(mesh, out);
}

bool write_obj(const Surface_mesh& mesh, std::ostream& ostr)
{
    char buffer[2048];

    // comment
    sprintf(buffer, "# OBJ export from Surface_mesh\n");
    ostr << buffer;
    ...
    for (Surface_mesh::Face_iterator fit=mesh.faces_begin(); fit!=mesh.faces_end(); ++fit)
    {
        ...
        do
        {
            if(with_tex_coord)
            {
                ...
                sprintf(buffer, " %d/%d/%d", (*fvit).idx()+1, (*fhit).idx()+1, (*fvit).idx()+1);
                ostr << buffer;
                ++fhit;
            }
            else if (normals)
            {
                ...
                sprintf(buffer, " %d//%d", (*fvit).idx()+1, (*fvit).idx()+1);
                ostr << buffer;
            }
            else {
                // write vertex index
                sprintf(buffer, " %d", (*fvit).idx()+1);
                ostr << buffer;
            }
        }
        while (++fvit != fvend);
...
public:
    ...
    CmpVec(real_t _eps=FLT_MIN) : eps_(_eps) {}
