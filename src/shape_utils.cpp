...
Shape *Shape::parseObj(char const *shapestr, Scheme shapescheme,
  ...
  while (!done) {
    ...
    case 'f':
      if (line[1] == ' ') {
        ...
        while ((nitems = sscanf(cp, "%d/%d/%d", &vi, &ti, &ni)) > 0) {
          ...
          if (nitems > 1)
            s->faceuvs.push_back(ti - 1);
          if (nitems > 2)
            s->facenormals.push_back(ni - 1);
          while (*cp && *cp != ' ')
            cp++;
          while (*cp == ' ')
            cp++;
    ...
    case 't':
      if (line[1] == ' ') {
        std::optional<Shape::tag> t = tag::parseTag(line);
        if (t)
...
//------------------------------------------------------------------------------
std::optional<Shape::tag> Shape::tag::parseTag(char const *line) {
  std::optional<Shape::tag> t;
  ...
  std::vector<real_t> tfloatargs;
  for (int i = 0; i < nfloats; ++i) {
    double val;
    while (*cp == ' ')
      cp++;
    if (sscanf(cp, "%lf", &val) != 1)
      return t;
  ...
  while (!done) {
    ...
    case 'n':
      ...
      if (sscanf(line, "newmtl %s", name) == 1) {
        mtls.emplace_back();
        mtl = &mtls[mtls.size() - 1];
        mtl->name = name;
      }
      break;
    case 'K':
      if (sscanf(line + 2, " %lf %lf %lf", &r, &g, &b) == 3) {
        switch (line[1]) {
        case 'a':
          assert(mtl);
          mtl->ka[0] = r;
    ...
    case 'i':
      ...
      break;
    case 's':
      if (sscanf(line, "sharpness %lf", &a) == 1) {
        assert(mtl);
        mtl->sharpness = a;
      }
      break;
  ...
  for (int i = 0, idx = 0; i < (int)nvertsPerFace.size(); ++i) {
    ...
    for (int j = 0; j < nvertsPerFace[i]; ++j) {
      int vert = faceverts[idx + j] + 1,
          ...
          normal =
              (int)facenormals.size() > 0 ? facenormals[idx + j] + 1 : vert;
      sh << vert << "/" << uv << "/" << normal << " ";
    }
    sh << "\\n\"\n";
    idx += nvertsPerFace[i];
...
std::string Shape::genObj() const {
  ...
  sh << "# This file uses centimeters as units for non-parametric "
        "coordinates.\n\n";
...
std::string Shape::genRIB() const {
  ...
  rib << "[";
  std::copy(nvertsPerFace.begin(), nvertsPerFace.end(),
            std::ostream_iterator<int>(rib));
  rib << "] ";

  rib << "[";
  std::copy(faceverts.begin(), faceverts.end(),
            std::ostream_iterator<int>(rib));
  rib << "] ";
  ...
  for (int i = 0; i < (int)tags.size();) {
    ...
    nargs << t->intargs.size() << " " << t->floatargs.size() << " "
          << t->stringargs.size();

    std::copy(t->intargs.begin(), t->intargs.end(),
              std::ostream_iterator<int>(intargs));

    std::copy(t->floatargs.begin(), t->floatargs.end(),
              std::ostream_iterator<float>(floatargs));

    std::copy(t->stringargs.begin(), t->stringargs.end(),
              std::ostream_iterator<std::string>(strargs));
