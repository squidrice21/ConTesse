...
public:
  ...
  inline std::vector<Intersection<Segment<Edge>> *> &intersections() {
...
public:
  ...
  inline void process(const Eigen::Vector2d &p,
                      const std::vector<Segment<Edge> *> &segments,
  ...
  std::vector<Segment<Edge> *> &intersectedEdges() { return m_intersectedEdges; }

  std::vector<Intersection<Segment<Edge>> *> &intersections() {
    return m_intersections;
  }
