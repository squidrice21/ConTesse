...
public:
  ...
  bool planeIntersect(const Vector3f &p, const Vector3f &norm,
...
protected:
  bool rayIntersectTri(const Ray &ray, uint32_t i, real_t &t,
                       Vector2f &uv) const;
  bool lineIntersectTri(const Ray &ray, uint32_t i) const;
  bool planeIntersectTri(const Vector3f &p, const Vector3f &norm,
                         uint32_t i) const;
